...
echo "samplename,description,phenotype" > ${ID}_prepare_samples.csv
for file in `ls $READS/lane/${ID}N*.gz`
do
  echo "$file,${ID}N,normal" >> ${ID}_prepare_samples.csv
done
for file in `ls $READS/lane/${ID}T*.gz`
do
  echo "$file,${ID}T,tumor" >> ${ID}_prepare_samples.csv
done
